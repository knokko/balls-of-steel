...
value class Spin internal constructor(val value: Float) : Comparable<Spin> {
	...
	operator fun div(right: Spin) = this.value.toDouble() / right.value.toDouble()
	...
	operator fun div(right: Duration) = toDouble(SpinUnit.RADIANS_PER_SECOND) * AngularAcceleration.RADPS2 / right.toDouble(DurationUnit.SECONDS)

	fun toSpeed(radius: Displacement) = Speed.METERS_PER_SECOND * (toDouble(SpinUnit.RADIANS_PER_SECOND) * radius.toDouble(DistanceUnit.METER))

	companion object {
		fun raw(value: Float) = Spin(value)

		val DEGREES_PER_SECOND = Spin(1.0f)
		val RADIANS_PER_SECOND = Spin(57.29577951308232f)
	}
}
